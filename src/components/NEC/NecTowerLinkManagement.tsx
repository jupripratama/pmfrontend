...
import { Plus, Edit, Trash, ArrowLeft, Building2, Radio } from "lucide-react";
                ...
                <>
                  ...
                  {towers.length === 0 && (
                    <Alert className="mt-4">
                      <AlertDescription>
                        Belum ada data tower. Klik tombol "Tambah Tower" untuk
                        menambahkan.
                      </AlertDescription>
                    </Alert>
                  )}
              ...
              {isLoading ? (
                ...
                <>
                  ...
                  {links.length === 0 && (
                    <Alert className="mt-4">
                      <AlertDescription>
                        Belum ada data link. Klik tombol "Tambah Link" untuk
                        menambahkan.
                      </AlertDescription>
                    </Alert>
                  )}
              ...
              <Input
                ...
                placeholder={
                  linkFormData.nearEndTowerId && linkFormData.farEndTowerId
                    ? `${towers.find(t => t.id === linkFormData.nearEndTowerId)?.name || ''} to ${towers.find(t => t.id === linkFormData.farEndTowerId)?.name || ''}`
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) => {
                    ...
                    if (!linkFormData.linkName.trim()) {
                      const nearTower = towers.find(t => t.id === newNearEndId);
                      const farTower = towers.find(t => t.id === linkFormData.farEndTowerId);
                      if (nearTower && farTower) {
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    {towers
                      .filter(tower => tower.id !== linkFormData.farEndTowerId) // ✅ Prevent same tower
                      .map((tower) => (
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) => {
                    ...
                    if (!linkFormData.linkName.trim()) {
                      const nearTower = towers.find(t => t.id === linkFormData.nearEndTowerId);
                      const farTower = towers.find(t => t.id === newFarEndId);
                      if (nearTower && farTower) {
                        setLinkFormData(prev => ({
                          ...prev,
                          linkName: `${nearTower.name} to ${farTower.name}`
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    {towers
                      .filter(tower => tower.id !== linkFormData.nearEndTowerId) // ✅ Prevent same tower
                      .map((tower) => (
                        <SelectItem key={tower.id} value={tower.id.toString()}>
                          {tower.name}
                        </SelectItem>
